...
from .reconstruct import reconstruct, reconstruct_minmod
...
"""
def riemann_solver2(left_state, right_state, velocity):
...
#     f_left_plus_eps, f_right_minus_eps = reconstruct_minmod(f, 2)
#     # f_left_minus_eps of i-th cell is f_right_minus_eps of the (i-1)th cell
#     f_left_minus_eps = af.shift(f_right_minus_eps, 0, 0, 1)
    
#     C_q1 = self.p1_center**3 / 2

#     f_left  = riemann_solver2(f_left_minus_eps, f_left_plus_eps, C_q1)
#     f_right = af.shift(f_left, 0, 0, -1)
    
#     left_flux  = multiply(C_q1, f_left)
#     right_flux = multiply(C_q1, f_right)

#     df_dt += -(right_flux - left_flux) / self.dq1
#     if(not at_n):
#         # Source term for the f_n evolution equation:
#         J1 = -1 * multiply(af.sum(multiply(f_left, self.p1_center), 0), self.dp1)
#         N_g = 3
#         # Applying periodic boundary conditions:
#         (self.fields_solver.yee_grid_EM_fields[0])[:, :, :N_g]  \
#         = (self.fields_solver.yee_grid_EM_fields[0])[:, :, -2 * N_g:-N_g]
#         (self.fields_solver.yee_grid_EM_fields[0])[:, :, -N_g:] \
#         = (self.fields_solver.yee_grid_EM_fields[0])[:, :, N_g:2*N_g]
        
#         E1_old = (self.fields_solver.yee_grid_EM_fields[0]).copy()
#         self.fields_solver.yee_grid_EM_fields[0] += -J1 * self.dt
#         af.eval(self.fields_solver.yee_grid_EM_fields[0])
        
#         E1_n_plus_half = 0.5 * (self.fields_solver.yee_grid_EM_fields[0] + E1_old)
...
#         df_dt   += -0.5 * multiply(self.p1_center, (multiply(f_left, E1_left) + multiply(f_right, E1_right)))

#         # Getting C_p at q1_left_p1_center:
#         C_p1_center = -0.5 * multiply(E1_n_plus_half, self.p1_center**2)
#         # Getting C_p at q1_left_p1_left:
#         C_p1_left   = -0.5 * multiply(E1_n_plus_half, self.p1_left**2)
#         # Getting C_p at q1_left_p1_right:
#         # C_p1_right  = 0.5 * (e / m) * E1_n_plus_half * p1_right**2

#         # Variation of p1 is along axis 0:
#         f_p1_left_plus_eps, f_p1_right_minus_eps = reconstruct_minmod(f, 0)
#         # f_left_minus_eps of i-th cell is f_right_minus_eps of the (i-1)th cell
#         f_p1_left_minus_eps = af.shift(f_p1_right_minus_eps, 1)
#         f_p1_left  = riemann_solver2(f_p1_left_minus_eps, f_p1_left_plus_eps, C_p1_center)
#         # For flux along p1 at q1_left:
#         flux_p1_left_at_q1_left  = multiply(C_p1_left, f_p1_left)
#         flux_p1_right_at_q1_left = af.shift(flux_p1_left_at_q1_left, -1) #C_p1_right * f_p1_right

#         d_flux_p1_at_q1_left  = flux_p1_right_at_q1_left - flux_p1_left_at_q1_left
#         d_flux_p1_at_q1_right = af.shift(d_flux_p1_at_q1_left, 0, 0, -1)

#         df_dt += -0.5 * multiply((d_flux_p1_at_q1_left + d_flux_p1_at_q1_right) , 1 / self.dp1)
#         af.eval(df_dt)

#         return(2 * multiply(df_dt, 1 / self.p1_center**2))
    
#     else:
#         E1_left  = (self.fields_solver.yee_grid_EM_fields[0])
#         E1_right = af.shift((self.fields_solver.yee_grid_EM_fields[0]), -1)
#         df_dt   += -0.5 * multiply(self.p1_center, (multiply(f_left, E1_left) + multiply(f_right, E1_right)))

#         # Getting C_p at q1_left_p1_center:
#         C_p1_center = -0.5 * multiply((self.fields_solver.yee_grid_EM_fields[0]), self.p1_center**2)
#         # Getting C_p at q1_left_p1_left:
#         C_p1_left   = -0.5 * multiply((self.fields_solver.yee_grid_EM_fields[0]), self.p1_left**2)
#         # Getting C_p at q1_left_p1_right:
