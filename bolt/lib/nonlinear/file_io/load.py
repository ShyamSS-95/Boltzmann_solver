...
# Load routines when using the deprecated style of HDF5 file writing:
# To be used only for developer testing. Prefer above methods for production:
def load_distribution_function_hdf5(self, file_name):
    """
    This function is used to load the distribution function from the
    dump file that was created by dump_distribution_function.
    Parameters
...
def load_EM_fields_hdf5(self, file_name):
    """
    This function is used to load the EM fields from the
    dump file that was created by dump_EM_fields.
    Parameters
